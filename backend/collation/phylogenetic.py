import numpy as np
import json
from scipy.cluster.hierarchy import linkage, dendrogram
from scipy.spatial.distance import squareform
import matplotlib.pyplot as plt
from io import BytesIO
import base64
from pymongo import MongoClient
from bson import ObjectId
import re
from collections import defaultdict


class PhylogeneticTreeBuilder:
    def __init__(self, mongo_uri='mongodb://localhost:27017/', db_name='document_db'):
        self.client = MongoClient(mongo_uri)
        self.db = self.client[db_name]
        self.documents = self.db['documents']
    
    def get_manuscript_info(self, ms_ids):
        """Get manuscript information for the given IDs."""
        manuscripts = {}
        for ms_id in ms_ids:
            try:
                doc = self.documents.find_one({"_id": ObjectId(ms_id)})
                if doc:
                    # Use sigla if available, otherwise use filename or ID
                    sigla = doc.get('metadata', {}).get('Sigla:') or doc.get('metadata', {}).get('Other Names:')
                    if not sigla:
                        sigla = doc.get('filename', str(ms_id)[-6:])
                    manuscripts[str(ms_id)] = {
                        'sigla': sigla,
                        'ms_id': str(ms_id)
                    }
            except Exception as e:
                print(f"Error getting info for manuscript {ms_id}: {e}")
                # Still include the manuscript with a generic name
                manuscripts[str(ms_id)] = {
                    'sigla': f"MS-{str(ms_id)[-6:]}",
                    'ms_id': str(ms_id)
                }
        return manuscripts
    
    def calculate_distance_matrix(self, collation_data, manuscripts):
        """
        Calculate distance matrix from collation data.
        
        Parameters:
        - collation_data: Dictionary mapping verse numbers to collation results
        - manuscripts: Dictionary of manuscript info
        
        Returns:
        - distance_matrix: 2D numpy array with pairwise distances
        - labels: List of manuscript labels corresponding to matrix indices
        """
        ms_ids = list(manuscripts.keys())
        n = len(ms_ids)
        
        # Create mapping from manuscript IDs to matrix indices
        ms_to_idx = {ms_id: idx for idx, ms_id in enumerate(ms_ids)}
        
        # Initialize distance matrix with zeros
        distance_matrix = np.zeros((n, n))
        
        # Create a matrix to track how many verses were compared for each pair
        comparison_counts = np.zeros((n, n))
        
        # Process each verse's collation data
        for verse_number, collation_result in collation_data.items():
            try:
                if isinstance(collation_result, str):
                    collation_result = json.loads(collation_result)
                
                # Extract witness data from the collation
                if 'witnesses' not in collation_result:
                    continue
                
                witnesses = collation_result['witnesses']
                
                # Get alignment table
                alignment_table = collation_result.get('table', [])
                if not alignment_table:
                    continue
                
                # For each pair of manuscripts
                for i, ms_id1 in enumerate(ms_ids):
                    for j in range(i+1, len(ms_ids)):
                        ms_id2 = ms_ids[j]
                        
                        # Convert from MongoDB ObjectId to string if needed
                        ms_id1_str = str(ms_id1)
                        ms_id2_str = str(ms_id2)
                        
                        # Determine if both manuscripts are represented in this verse's collation
                        w1_idx = -1
                        w2_idx = -1
                        
                        # In CollatEx output, we need to map witness indices
                        # Simple mapping: assume witnesses are in same order as input manuscripts
                        if len(witnesses) >= 2:
                            # This is a simplified approach; ideally we would track exactly 
                            # which manuscript corresponds to which witness
                            if i < len(witnesses):
                                w1_idx = i
                            if j < len(witnesses):
                                w2_idx = j
                        
                        if w1_idx >= 0 and w2_idx >= 0:
                            differences = 0
                            total_tokens = 0
                            
                            # Compare tokens in the alignment table
                            for row in alignment_table:
                                if w1_idx < len(row) and w2_idx < len(row):
                                    w1_token = row[w1_idx]
                                    w2_token = row[w2_idx]
                                    
                                    # Count differences (excluding None values)
                                    if w1_token is not None and w2_token is not None:
                                        if w1_token != w2_token:
                                            differences += 1
                                        total_tokens += 1
                            
                            # Add to distance matrix if we found tokens to compare
                            if total_tokens > 0:
                                distance = differences / total_tokens
                                distance_matrix[i, j] += distance
                                distance_matrix[j, i] += distance
                                comparison_counts[i, j] += 1
                                comparison_counts[j, i] += 1
            except Exception as e:
                print(f"Error processing verse {verse_number}: {e}")
        
        # Normalize distances by the number of verses compared
        # Add a small value to avoid division by zero
        with np.errstate(divide='ignore', invalid='ignore'):
            distance_matrix = np.divide(distance_matrix, comparison_counts, 
                                        out=np.zeros_like(distance_matrix), 
                                        where=comparison_counts!=0)
        
        # Fill in missing values (where no comparisons were made)
        # Use the average of all other distances
        mask = comparison_counts == 0
        if np.any(mask):
            # Calculate average of valid distances
            valid_distances = distance_matrix[~mask & ~np.eye(n, dtype=bool)]
            if len(valid_distances) > 0:
                avg_distance = np.mean(valid_distances)
                distance_matrix[mask] = avg_distance
        
        # Ensure diagonal is zero (distance to self)
        np.fill_diagonal(distance_matrix, 0)
        
        # Create labels for the matrix
        labels = [manuscripts[ms_id]['sigla'] for ms_id in ms_ids]
        
        return distance_matrix, labels
    
    def parse_collation_results(self, collation_results):
        """Parse collation results which may be strings or dictionaries."""
        parsed_results = {}
        for verse_number, result in collation_results.items():
            if isinstance(result, str):
                try:
                    parsed_results[verse_number] = json.loads(result)
                except json.JSONDecodeError:
                    print(f"Could not parse JSON for verse {verse_number}")
            else:
                parsed_results[verse_number] = result
        return parsed_results
    
    def generate_tree(self, collation_results, ms_ids, method='average', output_format='base64'):
        """
        Generate a phylogenetic tree from collation results.
        
        Parameters:
        - collation_results: Dictionary mapping verse numbers to collation results
        - ms_ids: List of manuscript IDs to include in the tree
        - method: Linkage method ('single', 'complete', 'average', 'weighted', 'centroid', 'median', 'ward')
        - output_format: Output format ('png', 'svg', 'base64')
        
        Returns:
        - tree_data: Tree visualization in the requested format
        """
        # Get manuscript information for labels
        manuscripts = self.get_manuscript_info(ms_ids)
        
        # Filter out manuscripts that don't exist in our info
        valid_ms_ids = [ms_id for ms_id in ms_ids if ms_id in manuscripts]
        
        if len(valid_ms_ids) < 3:
            raise ValueError("At least three valid manuscripts are required to build a tree")
        
        # Parse collation results if needed
        parsed_results = self.parse_collation_results(collation_results)
        
        # Calculate distance matrix
        distance_matrix, labels = self.calculate_distance_matrix(parsed_results, manuscripts)
        
        # Convert distance matrix to condensed form for linkage
        condensed_dist = squareform(distance_matrix)
        
        # Generate linkage matrix
        linked = linkage(condensed_dist, method=method)
        
        # Create dendrogram with a larger figure for more manuscripts
        fig_width = max(10, len(labels) * 0.5)
        fig_height = max(7, len(labels) * 0.3)
        plt.figure(figsize=(fig_width, fig_height))
        
        dendrogram(
            linked,
            orientation='right',
            labels=labels,
            distance_sort='descending',
            show_leaf_counts=True
        )
        plt.title(f'Manuscript Relationship Tree ({len(labels)} manuscripts)')
        plt.xlabel('Distance')
        
        # Adjust layout for better display
        plt.tight_layout()
        
        # Return the tree in the requested format
        if output_format == 'base64':
            buffer = BytesIO()
            plt.savefig(buffer, format='png', dpi=150, bbox_inches='tight')
            plt.close()
            buffer.seek(0)
            image_data = base64.b64encode(buffer.getvalue()).decode('utf-8')
            return image_data
        else:
            buffer = BytesIO()
            plt.savefig(buffer, format=output_format, dpi=150, bbox_inches='tight')
            plt.close()
            buffer.seek(0)
            return buffer.getvalue()
    
    def create_newick_tree(self, collation_results, ms_ids, method='average'):
        """
        Generate a Newick format tree string from collation results.
        
        Parameters:
        - collation_results: Dictionary mapping verse numbers to collation results
        - ms_ids: List of manuscript IDs to include in the tree
        - method: Linkage method ('single', 'complete', 'average', 'weighted', 'centroid', 'median', 'ward')
        
        Returns:
        - newick_string: Tree in Newick format
        """
        # Get manuscript information for labels
        manuscripts = self.get_manuscript_info(ms_ids)
        
        # Filter out manuscripts that don't exist in our info
        valid_ms_ids = [ms_id for ms_id in ms_ids if ms_id in manuscripts]
        
        if len(valid_ms_ids) < 3:
            raise ValueError("At least three valid manuscripts are required to build a tree")
        
        # Parse collation results if needed
        parsed_results = self.parse_collation_results(collation_results)
        
        # Calculate distance matrix
        distance_matrix, labels = self.calculate_distance_matrix(parsed_results, manuscripts)
        
        # Convert distance matrix to condensed form for linkage
        condensed_dist = squareform(distance_matrix)
        
        # Generate linkage matrix
        linked = linkage(condensed_dist, method=method)
        
        # Convert to Newick format
        def to_newick(node, labels, Z, n):
            if node < n:  # Leaf node
                return labels[node]
            else:  # Internal node
                node_idx = int(node - n)
                left = int(Z[node_idx, 0])
                right = int(Z[node_idx, 1])
                dist_left = Z[node_idx, 2] / 2
                dist_right = Z[node_idx, 2] / 2
                return f"({to_newick(left, labels, Z, n)}:{dist_left},{to_newick(right, labels, Z, n)}:{dist_right})"
        
        n = len(labels)
        newick = to_newick(2*n-2, labels, linked, n) + ";"
        
        return newick